...
export function neighbors(point) {
    ...
    return candidates.filter((candidate) => candidate.x >= 0 &&
        candidate.x <= MAX_INDEX &&
        candidate.y >= 0 &&
        candidate.y <= MAX_INDEX);
}
export function hasPath(edges) {
    const queue = [START];
...
export function buildEdges(rng) {
    const edges = new Set();
    const allEdges = [];
...
export function findRandomPath(edges, rng) {
    const queue = [START];
    const visited = new Set([`${START.x},${START.y}`]);
...
export function starPoints(center, outerRadius, innerRadius, spikes = 8) {
    const points = [];
...
export function pickSpreadTargets(pool, count, minDistance, rng) {
...
export function buildCellRegions(blockedEdges) {
        ...
        for (let x = 0; x < MAX_INDEX; x += 1) {
            ...
            while (queue.length > 0) {
                ...
                for (const next of neighbors) {
                    ...
                    if (blockedEdges.has(next.edge))
                        continue;
                    const nextKey = `${next.x},${next.y}`;
                    if (regions.has(nextKey))
                        continue;
...
export function edgesFromPath(path) {
    const edges = new Set();
...
export function regionCountForPath(path) {
...
function interiorNodeCount(path) {
    return path.filter((point) => point.x > 0 &&
        point.x < MAX_INDEX &&
        point.y > 0 &&
        point.y < MAX_INDEX).length;
}
function pickWildnessProfile(rng) {
    const presets = [
...
function pathWildnessScore(path, profile) {
    ...
    return (regions * profile.regionWeight +
...
export function buildLoopyPath(edges, rng, minLength, maxSteps) {
    let current = START;
    const path = [START];
    const usedEdges = new Set();
    const visitedNodes = new Set([`${START.x},${START.y}`]);
    for (let step = 0; step < maxSteps; step += 1) {
        const candidates = neighbors(current)
            .map((neighbor) => ({ neighbor, key: edgeKey(current, neighbor) }))
            .filter((candidate) => edges.has(candidate.key) &&
        ...
        if (path.length < minLength) {
            const withoutEnd = candidates.filter((candidate) => candidate.neighbor.x !== END.x || candidate.neighbor.y !== END.y);
        ...
        for (const candidate of options) {
            let weight = 1;
            const isInterior = candidate.neighbor.x > 0 &&
...
export function findBestLoopyPathByRegions(edges, rng, attempts, minLength, avoidSignatures) {
    ...
    if (candidatesBySignature.size === 0)
        return bestPath;
    ...
    const repeatFilteredPool = avoidSignatures && avoidSignatures.size > 0
    ...
    const minPoolScore = chosenPool.reduce((minScore, candidate) => Math.min(minScore, candidate.score), Number.POSITIVE_INFINITY);
