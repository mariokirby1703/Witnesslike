...
const SHAPES_BY_SIZE = (() => {
    ...
    for (const shape of POLYOMINO_SHAPES) {
        if (!bySize.has(shape.size))
            bySize.set(shape.size, []);
...
function rotationVariantsForShape(shape) {
    const key = canonicalRotationKey(shape.cells);
...
function listShapePlacements(shape, regionCells, regionSet) {
    const placements = [];
    const seen = new Set();
...
function listRequirementPlacements(symbol, regionCells, regionSet) {
...
function tileRegionWithShapes(regionCells, rng) {
    ...
    for (const shape of shapes) {
        ...
        for (const placement of placements) {
            for (const cell of placement.cells) {
                if (!placementsByCell.has(cell))
                    placementsByCell.set(cell, []);
    ...
    const solve = (remaining) => {
        ...
        const shuffled = shuffle(bestOptions, rng).sort((a, b) => preferLargeFirst ? b.shape.size - a.shape.size : a.shape.size - b.shape.size);
        for (const placement of shuffled) {
            if (!placement.cells.every((cell) => remaining.has(cell)))
                continue;
...
function canTileRegion(regionCells, symbols) {
    const regionSet = new Set(regionCells.map((cell) => `${cell.x},${cell.y}`));
    const regionSize = regionCells.length;
    const hasNegative = symbols.some((symbol) => symbol.negative);
    const useRelaxedIntroNegativeCoverage = hasNegative &&
        symbols.some((symbol) => symbol.shape.id.startsWith('intro-neg-relaxed-'));
    const positiveArea = symbols.reduce((sum, symbol) => sum + (symbol.negative ? 0 : symbol.shape.size), 0);
    const negativeArea = symbols.reduce((sum, symbol) => sum + (symbol.negative ? symbol.shape.size : 0), 0);
    if (!hasNegative && positiveArea !== regionSize)
        return false;
    if (hasNegative &&
    ...
    if (useRelaxedIntroNegativeCoverage) {
        ...
        const listRelaxedIntroPlacements = (symbol) => {
            const shapes = symbol.rotatable ? rotationVariantsForShape(symbol.shape) : [symbol.shape];
            const placements = [];
            const seen = new Set();
            for (const shape of shapes) {
                for (const anchor of boardCells) {
                    for (const shapeCell of shape.cells) {
                        ...
                        if (!absoluteCells.every((cell) => cell.x >= 0 &&
                            cell.x < MAX_INDEX &&
                            cell.y >= 0 &&
                            cell.y < MAX_INDEX)) {
                            continue;
        ...
        if (symbolPlacements.some((placements) => placements.length === 0))
            return false;
        const order = symbols
        ...
        const canStillReachExactCoverage = (startIndex) => {
            for (let cell = 0; cell < regionSize; cell += 1) {
                ...
                for (let i = startIndex; i < orderedPlacements.length; i += 1) {
                    if (orderedSigns[i] > 0 && positiveCoverageBySymbol[i][cell])
                        positivesLeft += 1;
                    if (orderedSigns[i] < 0 && negativeCoverageBySymbol[i][cell])
                        negativesLeft += 1;
                }
                const net = positiveCounts[cell] - negativeCounts[cell];
                const minNet = net - negativesLeft;
                const maxNet = net + positivesLeft;
                if (1 < minNet || 1 > maxNet)
                    return false;
            }
            return true;
        };
        const chosenPlacements = Array.from({ length: orderedPlacements.length }, () => null);
        const solve = (index) => {
            if (!canStillReachExactCoverage(index))
                return false;
            if (index >= orderedPlacements.length) {
                for (let cell = 0; cell < regionSize; cell += 1) {
                    if (positiveCounts[cell] - negativeCounts[cell] !== 1)
                        return false;
                }
                const positiveGlobalCells = new Set();
                for (let i = 0; i < chosenPlacements.length; i += 1) {
                    ...
                    if (!placement)
                        continue;
                    for (const cell of placement.globalCells)
                        positiveGlobalCells.add(cell);
                }
                for (let i = 0; i < chosenPlacements.length; i += 1) {
                    if (orderedSigns[i] >= 0)
                        continue;
            ...
            for (const placement of orderedPlacements[index]) {
                ...
                if (sign > 0) {
                    for (const cell of placement.regionCells)
                        positiveCounts[cell] += 1;
                }
                else {
                    for (const cell of placement.regionCells)
                        negativeCounts[cell] += 1;
                }
                if (solve(index + 1))
                    return true;
                if (sign > 0) {
    ...
    const symbolPlacements = symbols.map((symbol) => listRequirementPlacements(symbol, regionCells, regionSet).map((placement) => placement.cells
    ...
    if (!hasNegative) {
        const solveExact = (index, remaining) => {
            if (index >= orderedPlacements.length)
                return remaining.size === 0;
            for (const placement of orderedPlacements[index]) {
                if (!placement.every((cell) => remaining.has(cell)))
                    continue;
                const nextRemaining = new Set(remaining);
                for (const cell of placement)
                    nextRemaining.delete(cell);
                if (solveExact(index + 1, nextRemaining))
                    return true;
            }
            return false;
        };
...
function buildRegionCellsForPath(path) {
...
export function buildPolyominoPalette(rng, colorSquares, starsActive) {
    if (!starsActive)
        return [DEFAULT_POSITIVE_POLYOMINO_COLOR];
    const squareColors = Array.from(new Set(colorSquares.map((square) => square.color))).slice(0, 2);
    if (squareColors.length >= 2)
        return squareColors;
...
function generatePositivePolyominoSymbolsForEdges(edges, seed, minRegions, usedIconCells, colorPalette, rotatable, maxSymbols, preferredPath, blockedRegionIds) {
    if (maxSymbols <= 0)
        return null;
    const rng = mulberry32(seed);
    const solutionPath = preferredPath ?? findBestLoopyPathByRegions(edges, rng, 72, 8) ?? findRandomPath(edges, rng);
    if (!solutionPath)
        return null;
    ...
    let candidateRegions = shuffle(allRegions.filter((region) => region.cells.length <= 12 &&
        (!blockedRegionIds || !blockedRegionIds.has(region.id))), rng);
    if (candidateRegions.length === 0) {
        candidateRegions = shuffle(allRegions.filter((region) => !blockedRegionIds || !blockedRegionIds.has(region.id)), rng);
    }
    const additionalRegionBudget = candidateRegions.length >= 4 ? 3 : candidateRegions.length >= 2 ? 2 : 1;
    const targetRegions = Math.min(candidateRegions.length, minRegions + randInt(rng, additionalRegionBudget));
    if (candidateRegions.length < minRegions)
        return null;
    const symbols = [];
    const usedRegionIds = [];
    let placedRegions = 0;
    ...
    for (const region of candidateRegions) {
        if (placedRegions >= targetRegions)
            break;
        ...
        if (!tiling)
            continue;
        const availableCells = shuffle(region.cells.filter((cell) => !usedIconCells.has(`${cell.x},${cell.y}`)), rng);
        if (availableCells.length < tiling.length)
            continue;
        if (symbols.length + tiling.length > maxSymbols)
            continue;
    ...
    if (placedRegions < minRegions)
        return null;
    const minimumTotalArea = maxSymbols >= 3 ? 4 : maxSymbols === 2 ? 3 : 1;
    if (totalArea < minimumTotalArea)
        return null;
    return { symbols, solutionPath, usedRegionIds };
}
export function generatePolyominoesForEdges(edges, seed, minRegions, usedIconCells, colorPalette, maxSymbols = 4, preferredPath, blockedRegionIds) {
    return generatePositivePolyominoSymbolsForEdges(edges, seed, minRegions, usedIconCells, colorPalette, false, maxSymbols, preferredPath, blockedRegionIds);
}
export function generateRotatedPolyominoesForEdges(edges, seed, minRegions, usedIconCells, colorPalette, maxSymbols = 4, preferredPath, blockedRegionIds) {
    return generatePositivePolyominoSymbolsForEdges(edges, seed, minRegions, usedIconCells, colorPalette, true, maxSymbols, preferredPath, blockedRegionIds);
}
function pickNegativeShapeSize(rng, maxSize, minSize = 1, complexityBoost = 0) {
...
function pickNegativePairShape(regionCells, allowRotated, requireRotated, rng) {
    const regionSet = new Set(regionCells.map((cell) => `${cell.x},${cell.y}`));
    const maxSize = Math.min(4, regionCells.length);
    const baseComplexityBoost = regionCells.length >= 10
        ? 1
...
function pickPositivePairShape(regionCells, rotatable, rng, avoidCanonicalShapeKey) {
    const regionSet = new Set(regionCells.map((cell) => `${cell.x},${cell.y}`));
    const maxSize = Math.min(4, regionCells.length);
    for (let attempt = 0; attempt < 24; attempt += 1) {
...
export function generateNegativePolyominoesForEdges(edges, seed, positiveSymbols, usedIconCells, colorPalette, starsActive, pairedPositiveRotatable, allowRotatedNegative, requireRotatedNegative, maxNegativeSymbols = 4, maxExtraPositiveSymbols = 4, preferredPath) {
    if (maxNegativeSymbols <= 0 || maxExtraPositiveSymbols <= 0)
        return null;
    if (requireRotatedNegative && !allowRotatedNegative)
        return null;
    const rng = mulberry32(seed);
    const solutionPath = preferredPath ?? findBestLoopyPathByRegions(edges, rng, 72, 8) ?? findRandomPath(edges, rng);
    if (!solutionPath)
    ...
    for (const symbol of positiveSymbols) {
        ...
        if (regionId === undefined)
            continue;
        if (!regionSymbols.has(regionId))
            regionSymbols.set(regionId, []);
        regionSymbols.get(regionId)?.push(symbol);
    }
    const pairCapacityByCells = Array.from(regionFreeCells.values()).reduce((sum, free) => sum + Math.floor(free.length / 2), 0);
    ...
    if (pairLimit >= 3 && rng() < 0.4)
        targetPairs += 1;
    if (pairLimit >= 4 && rng() < 0.2)
        targetPairs += 1;
    targetPairs = Math.min(targetPairs, pairLimit);
    const negativeSymbols = [];
    const pairedPositiveSymbols = [];
    for (let attempt = 0; attempt < 72 && negativeSymbols.length < targetPairs; attempt += 1) {
        ...
        if (!cells || !free || free.length < 2)
            continue;
        const negativeShapeChoice = pickNegativePairShape(cells, allowRotatedNegative, requireRotatedNegative, rng);
        if (!negativeShapeChoice)
            continue;
        const positiveShapeChoice = pickPositivePairShape(cells, pairedPositiveRotatable, rng, canonicalRotationKey(negativeShapeChoice.shape.cells));
        if (!positiveShapeChoice)
            continue;
        const iconCells = shuffle([...free], rng).slice(0, 2);
        if (iconCells.length < 2)
            continue;
        const [positiveCell, negativeCell] = iconCells;
        const positiveColor = colorPalette[randInt(rng, colorPalette.length)];
        const nextPositiveSymbol = {
        ...
        regionFreeCells.set(regionId, free.filter((cell) => !(cell.x === positiveCell.x && cell.y === positiveCell.y) &&
            !(cell.x === negativeCell.x && cell.y === negativeCell.y)));
    }
    if (negativeSymbols.length < minDesiredPairs)
        return null;
    return { negativeSymbols, pairedPositiveSymbols, solutionPath };
}
export function checkPolyominoes(usedEdges, polyominoSymbols) {
    ...
    for (const [regionId, symbols] of polyByRegion.entries()) {
        ...
        if (!cells)
            return false;
        if (!canTileRegion(cells, symbols))
            return false;
    }
    return true;
}
