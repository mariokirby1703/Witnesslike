import { COLOR_PALETTE, buildCellRegions, edgesFromPath, findBestLoopyPathByRegions, findRandomPath, mulberry32, randInt, regionCountForPath, shuffle, } from '../puzzleUtils';
function pathSatisfiesColorSquares(path, squares) {
    if (path.length < 2)
        return false;
...
export function generateColorSquaresForEdges(edges, seed, desiredColorCount, selectedSymbolCount = 1, colorPool, preferredPath) {
    const availableColors = colorPool && colorPool.length > 0
        ? Array.from(new Set(colorPool))
        : COLOR_PALETTE;
    const effectiveColorCount = Math.max(1, Math.min(desiredColorCount, availableColors.length));
    const rng = mulberry32(seed);
    const path = preferredPath ?? findBestLoopyPathByRegions(edges, rng, 110, 10) ?? findRandomPath(edges, rng);
    if (!path)
        return null;
    const regionCount = regionCountForPath(path);
    if (regionCount < effectiveColorCount)
        return null;
    for (let attempt = 0; attempt < 36; attempt += 1) {
        ...
        if (regionIds.length < effectiveColorCount)
            continue;
        ...
        const regionList = shuffle(Array.from(regionCells.entries()).map(([id, cells]) => ({ id, cells })), localRng);
        const colorOrder = shuffle(Array.from({ length: colorCount }, (_, idx) => idx), localRng);
        ...
        for (; regionIndex < regionList.length; regionIndex += 1) {
            regionToColor.set(regionList[regionIndex].id, colorOrder[randInt(localRng, colorOrder.length)]);
        }
        const colorPools = Array.from({ length: colorCount }, () => []);
...
export function checkColorSquares(usedEdges, colorSquares) {
    const regions = buildCellRegions(usedEdges);
    const regionColors = new Map();
