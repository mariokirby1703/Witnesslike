...
export let START: Point = { x: 0, y: MAX_INDEX }
export let END: Point = { x: MAX_INDEX, y: 0 }
export let VIEWBOX = {
  x: -VIEWBOX_MARGIN,
  y: -VIEWBOX_MARGIN,
...
function clampPuzzleCellCount(cellCount: number) {
  return Math.max(
    MIN_PUZZLE_CELL_COUNT,
    Math.min(MAX_PUZZLE_CELL_COUNT, Math.floor(cellCount))
  )
}
...
export function setPuzzleCellCount(cellCount: number) {
